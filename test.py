...
def get_affordable(room, op_id, max_room=2):
    # TODO hack for ground truth affordable rooom
    affordable = False
    if op_id == 0:
        affordable = (room[0] > 0).item()
    elif op_id == 1:
        affordable = (room[1] > 0).item()
    elif op_id == 2:
        affordable = (room[0] < max_room).item()
    elif op_id == 3:
        affordable = (room[1] < max_room).item()
    return affordable
...
class MultistepAffordances(TrajectoryBufferWithBatchBase):
        ...
        for ep in range(num_epochs):
            for bix, (s_t, op_id, intent_target) in enumerate(get_batch(trajectory, trajectory_info)):
                ...
                if mask_used_op:
                    ...
                    # # =====
                    # TODO DEBUG
                    # if ep == (num_epochs - 1) and bix == 0:
                    #     sss, ooo, iii = s_t[:5], op_id[:5], intent_target[:5]
                    #     torch.where(sss == 1)
                    #     room = [x / 6 for x in torch.where(sss == 1)[2:]]
                    #     room = torch.stack(room).t()
                    #     aff = torch.ones(5, 4, device=room.device)
                    #     aff[:, 0].copy_(room[:, 0] > 0)
                    #     aff[:, 1].copy_(room[:, 1] > 0)
                    #     aff[:, 2].copy_(room[:, 0] < 2)
                    #     aff[:, 3].copy_(room[:, 1] < 2)
                    #
                    #     print("r", room)
                    #     print("aff", aff)
                    #     print("int", iii)
                    #     print("aff_s", aff.gather(1, op_id[:5].unsqueeze(1).long()))
                    #     print("pred", preds[:5])
                    #     print("op", op_id[:5])
                    #
                    # # =====
    ...
    def update_parameters(self) -> dict:
        ...
        while len(new_tr) < batch_trajectories or no_run:
            ...
            # # TODO DEBUG
            # for iqq in range(5):
            #     print(f"aff {tr[iqq][0].model_result.affordance.cpu().numpy().tolist()} room {tr[iqq][0].obs.room_pos.cpu().numpy().tolist()}")
            #
        ...
        # Print example off aff or interest
        # print(f"[INT]", new_tr[0][0].model_result.interest.cpu().numpy())
        # print(f"[AFF]", new_tr[0][0].model_result.affordance.cpu().numpy())
            ...
            for inds in self._get_batches_starting_indexes(batch_size, shift_start_idx=False):
                ...
                for i in range(recurrence):
                    ...
                    if termination_loss_coef > 0:
                        # TODO termination target is correct for the obs (and not offseted)
    ...
    def evaluate(self, eval_key=None) -> dict:
        ...
        while aff_op_cnt < eval_episodes:
            ...
            obs = next_obs
        # print(successfull_op, aff_op_cnt, bad_collected)
        # print(op_len)
